...
function save(next){if(storageError)throw Error('当前词库无法读取，已停止写入以保护原数据');saveData(localStorage,next);data=next;}
...
function render(){document.querySelectorAll('nav a').forEach(a=>a.classList.toggle('active',a.hash==='#'+page()));
 ...
 else if(page()==='settings')app.innerHTML=`<section class="hero"><h1>我的 Wordeer</h1><div class="muted">一点一滴，让英语成为日常。</div></section><section class="panel"><h3>词库与数据</h3><button class="settings-button" data-action="pending">待处理输入 <span class="muted">${data.pending.length} 条</span>　›</button><button class="settings-button" data-action="export">导出词库备份　↓</button><button class="settings-button" data-action="restore">导入并合并备份　↑</button><button class="settings-button" data-action="previous-backup">下载上一次自动备份　↓</button></section><section class="panel"><h3>关于网页测试版</h3><p class="muted">支持文字查词、自动分类、收藏、闪卡与选择题复习。词典包含 100,016 个词条（含短语）及词形还原。</p><p class="muted">数据保存在当前浏览器，暂不跨设备同步。清理网站数据前请导出备份。</p><p class="muted">网页录音使用浏览器语音识别服务，可能需要联网并由浏览器服务处理音频。Action Button 仍需原生 App。DeepSeek 尚未连接，未命中的词会进入待处理。</p><button class="link" data-action="licenses">开源词典与字体许可</button></section>`;
...
function advance(){if(!review||!review.graded)return;const next=structuredClone(review);next.index++;next.revealed=false;next.answered=false;next.graded=false;const word=next.words[next.index];if(word)next.options=makeOptions(word,data.words);save({...data,reviewSession:next});review=next;renderReview();}
